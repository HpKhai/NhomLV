...
const AdminStore = () => {
    ...
    const mutationUpdate = useMutationHooks(
        (data) => {
            const {
                ...
                token,
                ...rests } = data
            const res = StoreService.updateStore(
                id,
                token,
                rests
            )
    ...
    const mutationDelete = useMutationHooks(
        (data) => {
            const {
                id,
                token } = data
            const res = StoreService.deleteStore(
                id,
                token)
            return res
        }
    ...
    const mutationDeleteMany = useMutationHooks(
        (data) => {
            ...
            const res = StoreService.deleteManyStore(
                ids,
                token)
            return res
        }
    ...
    const renderAction = () => {
        return (
            <div>
                ...
                <EditOutlined style={{ color: 'blue', fontSize: '16px', cursor: 'pointer', margin: '5px' }} onClick={handleDetailsStore} />
            </div>
        )
    }
    ...
    const dataTable = Stores?.data?.length && Stores?.data?.map((Store) => {
    ...
    const onUpdateStore = () => {
        mutationUpdate.mutate({ id: rowSelected, token: user?.access_token, ...stateStoreDetails }, {
            onSettled: () => {
                queryStore.refetch()
            }
        })
        ...
        <div>
            ...
            <ModalComponent forceRender title="Tạo cửa hàng" open={isModalOpen} onCancel={handleCancel} footer=''  >
                ...
                >
                    ...
                    >
                        <InputComponents value={stateStore.x} onChange={handleOnChange} name="x" />
                    </Form.Item>
                    ...
                    >
                        <InputComponents value={stateStore.y} onChange={handleOnChange} name="y" />
                    </Form.Item>
            ...
            <DrawerComponent title='Chi tiết cửa hàng' isOpen={isOpenDrawer} onClose={() => setIsOpenDrawer(false)} width='75%' >
                <Form
                    name="update"
                    labelCol={{ span: 4 }}
                    wrapperCol={{ span: 20 }}
                ...
                >
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Please input name store!' }]}
                    >
                        <InputComponents value={stateStoreDetails.name} onChange={handleOnChangeDetails} name="name" />
                    </Form.Item>
                    ...
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Please input x of store!' }]}
                    >
                        <InputComponents value={stateStoreDetails.x} onChange={handleOnChangeDetails} name="x" />
                    </Form.Item>
                    ...
                    <Form.Item
                        ...
                        rules={[{ required: true, message: 'Please input y of store!' }]}
                    >
                        <InputComponents value={stateStoreDetails.y} onChange={handleOnChangeDetails} name="y" />
                    </Form.Item>
            ...
            <ModalComponent forceRender title="Xóa sản phẩm" open={isModalOpenDelete} onCancel={handleCancelDelete} onOk={handleDeleteStore} >
                <div>
                    {`Bạn có muốn xóa cửa hàng không?`}
                </div>
            </ModalComponent>
        </div>
    )
